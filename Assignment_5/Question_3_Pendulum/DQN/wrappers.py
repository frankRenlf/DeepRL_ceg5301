...
class GenerateFrame42(gym.ObservationWrapper):
    def __init__(self, env=None):
        ...
        self.observation_space = spaces.Box(
            low=0, high=255, shape=(self.screen_dim, self.screen_dim, 1)
        )
    ...
    def get_image_data(self):
        ...
        rod_end = pygame.math.Vector2(rod_end).rotate_rad(
            self.unwrapped.state[0] + np.pi / 2
        )
...
class FrameStack(gym.Wrapper):
    ...
    def _get_obs(self):
        ...
        return [
            LazyFrames(list(self.frames)).__array__(),
            LazyArraies(self.states).__array__(),
        ]
...
def wrap_cover_pendulun_test(
    env_name, disc_actions, render_mode=None, episode_length=200
):
    def wrap_():
        """Apply a common set of wrappers for Atari games."""
        print("NAME: ", env_name)
